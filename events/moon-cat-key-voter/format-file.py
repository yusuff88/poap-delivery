...
MOON_CAT_EVENT_ID = 1413
original_name = 'original.txt'

def main():
    print('> Starting MoonCat Key Voter')
    formatted_output = {}
    addresses_set = set()
    with open(original_name, encoding='utf-8-sig') as original_file:
        for each in original_file:
            ...
            formatted_output[address] = [MOON_CAT_EVENT_ID, ]

    with open('output.json', 'w') as outfile:
        json.dump(formatted_output, outfile, indent=4)

    original_file_length = sum(1 for line in open(original_name, 'r') if line != '\n')
    original_file_without_duplicates_length = len(set(line for line in open(original_name, 'r') if line != '\n'))
    print('')
